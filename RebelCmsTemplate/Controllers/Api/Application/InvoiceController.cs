...
public class InvoiceController : Controller {
 ...
 public async Task<IActionResult> Get()
 {
  ...
  public ActionResult Post()
  {
	...
	int customerKey =  !string.IsNullOrEmpty(Request.Form["customerKey"])?Convert.ToInt32(Request.Form["customerKey"]):0;
	int shipperKey =  !string.IsNullOrEmpty(Request.Form["shipperKey"])?Convert.ToInt32(Request.Form["shipperKey"]):0;
	int employeeKey =  !string.IsNullOrEmpty(Request.Form["employeeKey"])?Convert.ToInt32(Request.Form["employeeKey"]):0;
	DateOnly invoiceOrderDate = DateOnly.FromDateTime(DateTime.Now);
	if (!string.IsNullOrEmpty(Request.Form["invoiceOrderDate"]))
	{
	var dateString = Request.Form["invoiceOrderDate"].ToString().Split("-");
	invoiceOrderDate = new DateOnly(Convert.ToInt32(dateString[0]), Convert.ToInt32(dateString[1]), Convert.ToInt32(dateString[2]));
	}
	DateOnly invoiceRequiredDate = DateOnly.FromDateTime(DateTime.Now);
	if (!string.IsNullOrEmpty(Request.Form["invoiceRequiredDate"]))
	{
	var dateString = Request.Form["invoiceRequiredDate"].ToString().Split("-");
	invoiceRequiredDate = new DateOnly(Convert.ToInt32(dateString[0]), Convert.ToInt32(dateString[1]), Convert.ToInt32(dateString[2]));
	}
	DateOnly invoiceShippedDate = DateOnly.FromDateTime(DateTime.Now);
	if (!string.IsNullOrEmpty(Request.Form["invoiceShippedDate"]))
	{
	var dateString = Request.Form["invoiceShippedDate"].ToString().Split("-");
	invoiceShippedDate = new DateOnly(Convert.ToInt32(dateString[0]), Convert.ToInt32(dateString[1]), Convert.ToInt32(dateString[2]));
	}
	decimal invoiceFreight =  !string.IsNullOrEmpty(Request.Form["invoiceFreight"])?Convert.ToDecimal(Request.Form["invoiceFreight"]):0;
