...
namespace RebelCmsTemplate.Controllers
{
    ...
    public class RouteController : Controller
    {
        ...
        public async Task<IActionResult> Get()
        {
            ...
            var page = await _renderViewAsStringUtil.RenderViewToStringAsync(ControllerContext, templatePath, ("Foo", "Bar"));
            return Ok(page);
        }
        [HttpPost]
        public async Task<IActionResult> Post()
        {
            ...
            if (access)
            {
                await using (MySqlConnection connection = SharedUtil.GetConnection())
                {
                    ...
                    try
                    {
                        ...
                        await using (var reader = mySqlCommand.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                var templatePath = "~/Views/Pages/" + reader["folderName"]?.ToString()?.Trim() + "/" + reader["leafFilename"].ToString()?.Trim() + ".cshtml";
                                page = await _renderViewAsStringUtil.RenderViewToStringAsync(ControllerContext, templatePath);
                                status = true;
                            }
                        }
