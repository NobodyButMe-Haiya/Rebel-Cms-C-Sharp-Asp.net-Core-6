...
namespace RebelCmsTemplate.Util
{
    public class MenuUtil
    {
        ...
        public List<MenuModel> GetMenu()
        {
            ...
            const string sqlFolder = "SELECT * FROM folder_access JOIN folder USING (folderId) WHERE folder.isDelete != 1 AND roleId = @roleId ";
            const string sqlLeaf = "SELECT * FROM leaf_access JOIN leaf USING (leafId) WHERE leaf.isDelete != 1 AND roleId = @roleId AND folderId=@folderId";
            ...
            try
            {
               ...
               mySqlCommand.Parameters.AddWithValue("@roleId", _sharedUtil.GetRoleId());
                ...
                foreach (FolderAccessModel folderAccessModel1 in _folderAccessModels)
                {
